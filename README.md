# Symptom
This is the GitHub page for the project for the class of "Advanced Software Engineering Techniques 2024-2025"
Group composition:  
	(Scrum Master)Catîru Ștefan -> [SOA](/sota_Stefan.md), [Github](https://github.com/StefanUAIC), Discord -> @stefan696969  
	Maftei Briana Stefania -> [SOA](/sota_Briana.md), [Github](https://github.com/brianamaftei), Discord ->  @brianastefania  
	Maxim Matei -> [SOA](/sota_Matei.md), [Github](https://github.com/HowDidThat), Discord -> @howdidthat
  
### Coordinator
Olariu Maria

Faculty of Computer Science

Alexandru Ioan Cuza University

Iasi, Romania
   
# [Tasks](/tasks.md)
# [Useful Links](/links.md)



	


